...
class Capybara::Driver::EnvjsTrr < Capybara::Driver::Base
  ...
  def initialize(app)
    ...
    #     e.merge! "HTTP_X_REQUESTED_WITH" => xhr.headers["X-Requested-With"] if xhr.headers["X-Requested-With"]
...
Capybara.register_driver :envjs_trr do |app|
  Capybara::Driver::EnvjsTrr.new(app)
end
